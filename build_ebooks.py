    ...
    <div id="chaptersModalOverlay" class="fixed inset-0 bg-slate-950/85 backdrop-blur-md z-50 flex items-start sm:items-center justify-center opacity-0 pointer-events-none p-3 sm:p-4 overflow-y-auto pt-6 sm:pt-4">
        <div id="chaptersModalCard" class="w-full max-w-2xl bg-slate-900 border border-slate-700 rounded-2xl p-4 sm:p-5 shadow-2xl flex flex-col my-auto max-h-[88vh] sm:max-h-[85vh]">
            ...
            <div class="overflow-y-auto space-y-3.5 pr-1 flex-1 min-h-0 custom-scrollbar">
                ...
                <!-- 1. STATS CHAPTER AT BEGINNING (Defaults to Collapsed, Darker Charcoal Gray Container) -->
                <div class="bg-gray-950 border border-slate-700/80 rounded-xl overflow-hidden shadow-lg ring-1 ring-slate-800/60">
                    <button id="toggleStatsAccordionBtn" type="button" class="w-full p-3 flex items-center justify-between text-left bg-gray-950 hover:bg-gray-900/90 transition-colors select-none">
                    ...
                    <!-- Collapsed Body Content (Darker Gray Background) -->
                    <!-- Collapsed Body Content (Darker Gray Background) -->
                    <div id="statsAccordionBody" class="hidden p-3.5 pt-2 border-t border-slate-800/90 bg-gray-950/70 space-y-3">
                        ...
                        <div id="chartViewSession" class="w-full space-y-3">
                            ...
                            <div class="bg-slate-900/90 p-3.5 sm:p-4 rounded-xl border border-slate-800 shadow-md space-y-3">
                                <div class="flex items-center justify-between text-xs font-semibold text-slate-300 border-b border-slate-800/80 pb-2.5">
                                    ...
                                    <span id="telemetryPersonalStatus" class="text-[10px] font-mono px-2 py-0.5 rounded bg-slate-950 border border-slate-800 text-slate-400">
                                        Reading...
                                    </span>
                                </div>
                                ...
                                <div class="grid grid-cols-2 sm:grid-cols-4 gap-2 text-center">
                                    <!-- 1. Unique Readers -->
                                    <div class="bg-slate-950/80 p-2 sm:p-2.5 rounded-lg border border-slate-800/80">
                                ...
                                <div class="bg-slate-950/90 p-3 rounded-xl border border-slate-800/80 space-y-2.5">
                                    <div class="flex items-center justify-between text-[11px] text-slate-300 font-medium">
                                        ...
                                        <span id="telemetryJourneyTargetText" class="text-[10px] font-mono text-emerald-400 font-semibold">Target: 100% Finisher Circle</span>
                                    </div>
                                    ...
                                    <div id="telemetryMotivationBanner" class="text-[11px] p-2.5 rounded-lg bg-slate-900 border border-slate-800 text-slate-300 leading-normal flex items-center gap-2">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-2.5">
                                ...
                                <div class="bg-slate-900/90 p-3 sm:p-3.5 rounded-xl border border-slate-800 flex flex-col justify-between gap-2.5 shadow-sm">
                                    <div>
    ...
    <script>
        (function() {{
            ...
            function updateTelemetryUI() {{
                ...
                const targetText = document.getElementById('telemetryJourneyTargetText');
                ...
                if (userProgressFill) userProgressFill.style.width = displayUserDepth + '%';
                ...
                if (targetText) {{
                    targetText.textContent = compPct + '% Finisher Rate (' + cCount + '/' + rCount + ' readers)';
                }}
                ...
                if (motivationText && motivationBanner) {{
                    if (isCompleted) {{
                        ...
                        motivationText.innerHTML = '<strong>🏆 Book Completed!</strong> You are part of the ' + compPct + '% finisher circle (' + cCount + '/' + rCount + ' readers).';
                        ...
                        motivationText.innerHTML = '<strong>🔥 Great Momentum!</strong> You are at <strong>' + myDepth + '% depth</strong>, past the average drop-off point (~' + avgDropoffPct + '%). Push forward to finish!';
                    }} else if (myDepth > 0) {{
                        ...
                        motivationText.innerHTML = '<strong>📖 Keep Reading:</strong> You are at ' + myDepth + '% depth. Read past the ~' + (avgDropoffPct || 45) + '% mark to join the ' + compPct + '% finisher circle!';
                    }} else {{
                        motivationBanner.className = "text-[11px] p-2.5 rounded-lg bg-slate-900 border border-slate-800 text-slate-300 leading-normal flex items-center gap-2";
                        motivationText.textContent = 'Scroll through the story to advance your reading progress toward the 100% completion goal.';
                    }}
                }}

                if (statusEl) {{
                    if (isCompleted) {{
                        statusEl.innerHTML = '<span class="text-emerald-400 font-bold">✨ Finisher</span>';
                    }} else {{
                        statusEl.innerHTML = '<span class="text-amber-400">📖 ' + myDepth + '% Read</span>';
                    }}
                }}
            }}
            updateTelemetryUI();
