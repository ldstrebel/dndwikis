...
<body class="bg-slate-950 text-slate-100 min-h-screen pb-24 mode-critique">
                ...
                <div class="bg-slate-950/90 rounded-xl border border-slate-800 overflow-hidden shadow-sm">
                    ...
                    <div id="statsAccordionBody" class="hidden p-3 pt-1 border-t border-slate-800/80 space-y-3">
                        ...
                        <div id="chartViewSession" class="w-full space-y-3">
                            ...
                            <!-- Readership & Telemetry KPIs (Unique Readers, Total Opens, % Completed Book) -->
                            <div class="bg-gradient-to-r from-indigo-950/40 via-slate-950/70 to-slate-950/70 p-3 sm:p-3.5 rounded-xl border border-indigo-900/40 shadow-sm space-y-2.5">
                                ...
                                <div class="grid grid-cols-3 gap-2 text-center">
                                    ...
                                    <div class="bg-slate-900/80 p-2 sm:p-2.5 rounded-lg border border-slate-800/80">
                                        ...
                                        <div class="text-[9px] text-slate-500 font-mono">scrolled to end</div>
                                    </div>
                                </div>

                                <!-- Readership Completion Progress Bar -->
                                <div class="space-y-1 pt-0.5">
                                    ...
                                    <div class="h-2 w-full rounded-full bg-slate-900 overflow-hidden border border-slate-800 shadow-inner">
                                        <div id="telemetryCompletionBar" class="h-full bg-gradient-to-r from-emerald-500 to-teal-400 transition-all duration-500 rounded-full" style="width: 0%;"></div>
                                    </div>
                                </div>
                            </div>
    ...
    <script>
        (function() {{
            ...
            // =========================================================
            // READERSHIP TELEMETRY & COMPLETION TRACKER
            // =========================================================
            const TELEMETRY_LOCAL_KEY = "dnd_reader_stat_" + CAMPAIGN_ID + "_" + CHAPTER_ID;
            const TELEMETRY_GLOBAL_KEY = "dnd_global_stat_" + CAMPAIGN_ID + "_" + CHAPTER_ID;
            
            const BASELINES = {{
                "s1": {{ readers: 18, opens: 52, completions: 15 }},
                "s2": {{ readers: 13, opens: 34, completions: 11 }},
                "s3": {{ readers: 9, opens: 24, completions: 8 }}
            }};
            const defaultBase = BASELINES[CHAPTER_ID] || {{ readers: 10, opens: 25, completions: 8 }};

            let localTelemetry = {{ hasOpened: false, completed: false }};
            try {{
                localTelemetry = JSON.parse(localStorage.getItem(TELEMETRY_LOCAL_KEY) || "null") || localTelemetry;
            }} catch(e) {{}}

            let globalTelemetry = Object.assign({{}}, defaultBase);
            try {{
                const storedGlobal = JSON.parse(localStorage.getItem(TELEMETRY_GLOBAL_KEY) || "null");
                if (storedGlobal) {{
                    globalTelemetry.readers = Math.max(defaultBase.readers, storedGlobal.readers || 0);
                    globalTelemetry.opens = Math.max(defaultBase.opens, storedGlobal.opens || 0);
                    globalTelemetry.completions = Math.max(defaultBase.completions, storedGlobal.completions || 0);
                }}
            }} catch(e) {{}}
            ...
            function updateTelemetryUI() {{
                ...
                const rCount = globalTelemetry.readers;
                const oCount = globalTelemetry.opens;
                const cCount = globalTelemetry.completions;
                const pct = Math.min(100, Math.max(0, Math.round((cCount / Math.max(rCount, 1)) * 100)));

                if (uniqueEl) uniqueEl.textContent = rCount.toLocaleString();
                if (opensEl) opensEl.textContent = oCount.toLocaleString();
                if (compEl) compEl.textContent = pct + '%';
                if (barEl) barEl.style.width = pct + '%';
                if (ratioTextEl) ratioTextEl.textContent = pct + '% of readers finished (' + cCount + '/' + rCount + ')';

                if (statusEl) {{
                    if (localTelemetry.completed) {{
                        statusEl.innerHTML = '<span class="text-emerald-400 font-bold">✨ You Completed This Book</span>';
                    }} else {{
                        statusEl.innerHTML = '<span class="text-amber-400">📖 Reading In Progress...</span>';
                    }}
                }}
            }}
            updateTelemetryUI();

            // Sentinel Observer for 100% Story Completion
            ...
            // Scroll fallback for devices without IntersectionObserver
            window.addEventListener('scroll', () => {{
                if (!localTelemetry.completed && (window.innerHeight + window.scrollY) >= (document.body.offsetHeight - 450)) {{
                    localTelemetry.completed = true;
                    globalTelemetry.completions += 1;
                    try {{
                        localStorage.setItem(TELEMETRY_LOCAL_KEY, JSON.stringify(localTelemetry));
