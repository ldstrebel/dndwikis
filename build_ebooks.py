    ...
    <script>
        (function() {{
            ...
            function updateTopProgressBar() {{
                ...
                // Update breadcrumb milestones matching the active gradient (Cyan in-progress -> Emerald on completion)
                const isCompleted = (isFinishedBefore || progress >= 95);
                ...
                breadcrumbs.forEach(dot => {{
                    ...
                    const isTraversed = (isCompleted && pct <= 100) || (maxDepth >= pct);
                    if (isTraversed) {{
                        if (isCompleted) {{
                            // Finished: Glowing Emerald matching completion gradient & pip
                            ...
                            // In-Progress: Matches the initial blue-sky-cyan reading gradient
                            dot.className = "w-1.5 h-1.5 rounded-full bg-cyan-300 shadow-[0_0_6px_rgba(103,232,249,0.95)] transition-all duration-300 scale-110";
                        }}
                    }} else {{
